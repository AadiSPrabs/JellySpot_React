...
interface PlayerState {
  ...
  toggleShuffle: (keepCurrentTrackFirst?: boolean) => void;
  toggleRepeat: () => void;
...
const getEffectiveAudioQuality = async (): Promise<
  "lossless" | "high" | "low"
> => {
  ...
  if (
    cachedNetworkState &&
    now - cachedNetworkState.timestamp < NETWORK_CACHE_TTL
  ) {
  ...
  try {
    ...
    logError(
      "PlayerStore",
      `Failed to detect network type: ${getErrorMessage(error)}`,
    );
...
const HEAVY_FIELDS = new Set(['lyrics', 'streamUrl']);
    ...
    if (state.queue.length > 500) {
      const currentIndex = state.currentTrack
        ? state.queue.findIndex(
            (t) =>
              (t.queueItemId || t.id) ===
              (state.currentTrack?.queueItemId || state.currentTrack?.id),
          )
        : 0;
...
export const usePlayerStore = create<PlayerState>((set, get) => ({
  ...
  init: async () => {
    ...
    const sub2 = TrackPlayer.addEventListener(
      ...
      async (event: any) => {
        ...
        const currentIndex = currentQueue.findIndex(
          (t) => (t.queueItemId || t.id) === (ct.queueItemId || ct.id),
        );
    ...
    const sub3 = TrackPlayer.addEventListener(
      ...
      async (event: any) => {
        ...
        const newTrack = queue.find(
          (t) =>
            (t.queueItemId || t.id) === event.track.id ||
            t.streamUrl === event.track.url,
        );
        if (newTrack) {
          ...
          try {
            ...
            logError(
              "PlayerStore",
              `Failed to prune native queue: ${getErrorMessage(e)}`,
            );
          }
          ...
          if (previousTrack && previousTrack.id !== newTrack.id) {
            const source = previousTrack.streamUrl?.startsWith("file://")
              ? "local"
              : "jellyfin";
            const listenDuration =
              previousPositionMillis > 0
                ? previousPositionMillis
                : previousTrack.durationMillis || 0;
            DatabaseService.recordPlay(
              previousTrack,
              source,
              listenDuration,
              previousPositionMillis >
                (previousTrack.durationMillis || 0) * 0.8,
              previousTrack.playlistId,
            ).catch(() => {});
        ...
        setTimeout(async () => {
          ...
          const currentIndex = currentQueue.findIndex(
            (t) =>
              (t.queueItemId || t.id) === (newTrack.queueItemId || newTrack.id),
          );
          ...
          if (nextTrack) {
            try {
              ...
              if (
                !nextNativeTrack ||
                nextNativeTrack.id !== (nextTrack.queueItemId || nextTrack.id)
              ) {
                ...
                if (nextUrl) {
                  await audioService.addToQueue({
                    ...
                    artwork:
                      nextTrack.imageUrl &&
                      !nextTrack.imageUrl.startsWith("data:")
                        ? nextTrack.imageUrl
                        : undefined,
                    duration: Math.max((nextTrack.durationMillis || 0) / 1000, 1),
                  });
                }
              }
            } catch (e) {
    ...
    const sub4 = TrackPlayer.addEventListener(
      Event.PlaybackState,
      (event: any) => {
        ...
        set((state) => {
          ...
          if (state.isPlaying !== isPlayingState)
            updates.isPlaying = isPlayingState;
          if (state.isBuffering !== isBufferingState)
            updates.isBuffering = isBufferingState;
    ...
    const sub5 = TrackPlayer.addEventListener(
      ...
      async (event: any) => {
        ...
        if (
          sleepTimerTarget &&
          typeof sleepTimerTarget === "number" &&
          Date.now() > sleepTimerTarget
        ) {
        ...
        const newDuration =
          event.duration > 0 ? event.duration * 1000 : existingDuration;
        ...
        if (currentTrack && !currentTrack.streamUrl?.startsWith("file://")) {
          ...
          if (
            !lastServerProgressUpdate ||
            now - lastServerProgressUpdate >= syncThrottle
          ) {
          ...
          if (!savedState?.currentTrack) {
            ...
            if (currentNativeIndex !== undefined) {
              const currentTrackObj =
                await TrackPlayer.getTrack(currentNativeIndex);
              const nativeQueue = await TrackPlayer.getQueue();
              ...
              if (currentTrackObj) {
                ...
                const mappedQueue: Track[] = (nativeQueue || []).map(
                  (t: any) => ({
                    id: t.id,
                    name: t.title,
                    artist: t.artist,
  ...
  playTrack: async (track) => {
    ...
    if (!remoteState.isTargetLocal()) {
      ...
      if (sessionId) {
        ...
        webSocketService.sendCommand(sessionId, "Play", {
          ItemIds: [track.id],
        });
        return;
      }
    ...
    if (currentIndex === -1) {
      set({
        queue: [trackWithId],
        originalQueue: [trackWithId],
        currentTrack: trackWithId,
        isPlaying: true,
        positionMillis: 0,
        durationMillis: trackWithId.durationMillis || 0,
      });
      currentIndex = 0;
    } else {
      set({
    ...
    try {
      ...
      const idx = currentQueue.findIndex(
        (t) =>
          (t.queueItemId || t.id) ===
          (trackWithId.queueItemId || trackWithId.id),
      );
      let nextTrack: Track | undefined;
      if (repeatMode === "one") {
        nextTrack = track;
        ...
        setTimeout(async () => {
          try {
            ...
            await audioService.addToQueue({
              ...
              artist: nt.artist,
              artwork:
                nt.imageUrl && !nt.imageUrl.startsWith("data:")
                  ? nt.imageUrl
                  : undefined,
              duration: Math.max((nt.durationMillis || 0) / 1000, 1),
            });
          } catch (e) {
            /* silent */
          }
  ...
  setRawQueue: (
    ...
    const mapped = rawTracks
      .slice(0, queueLimit > 0 ? queueLimit : undefined)
      .map((t: any) => ({
        id: t.Id || t.id,
        name: t.Name || t.name,
        ...
        imageUrl:
          ...
          t.imageUrl ||
          (dataSource === "jellyfin"
            ? `${serverUrl}/Items/${t.Id}/Images/Primary?api_key=${token}`
            : ""),
        imageBlurHash: t.ImageBlurHashes?.Primary
          ? (Object.values(t.ImageBlurHashes.Primary)[0] as string)
          : undefined,
        durationMillis: t.RunTimeTicks
          ? t.RunTimeTicks / 10000
          : t.durationMillis || 0,
        ...
        codec: isLocal
          ? t.codec
          : t.MediaSources?.[0]?.Codec ||
            t.MediaSources?.[0]?.MediaStreams?.find(
              (s: any) => s.Type === "Audio",
            )?.Codec,
  ...
  removeFromQueue: (queueItemId) => {
    ...
    set({
      ...
      originalQueue: originalQueue.filter(
        (t) => (t.queueItemId || t.id) !== queueItemId,
      ),
    });
    persistQueueState();
  },
  ...
  playNext: async () => {
    ...
    if (!remoteState.isTargetLocal()) {
      ...
      if (sessionId) {
        set({
          ...
          positionMillis: 0,
          durationMillis: targetTrack.durationMillis || 0,
        });
    ...
    try {
      ...
      if (
        currentNativeIndex !== undefined &&
        nativeQueue &&
        nativeQueue.length > 0
      ) {
        const targetNativeIndex = nativeQueue.findIndex(
          (t) => t.id === (targetTrack.queueItemId || targetTrack.id),
        );

        // Only skip if found AND it's different (unless repeating one)
        if (
          targetNativeIndex !== -1 &&
          (targetNativeIndex !== currentNativeIndex || repeatMode === "one")
        ) {
  ...
  seek: async (positionMillis) => {
    ...
    if (!remoteState.isTargetLocal()) {
      ...
      if (sessionId) {
        set({ positionMillis });
        webSocketService.sendCommand(sessionId, "Seek", {
  ...
  toggleShuffle: (keepCurrentTrackFirst = true) => {
    ...
    if (newMode) {
      ...
      // (skip for "shuffle all" scenarios where we want a fresh track)
      if (keepCurrentTrackFirst && currentTrack) {
      ...
      // Respect queueLimit
      const limited =
        queueLimit > 0 && shuffled.length > queueLimit
