...
import { sqliteDb as db } from '../db/client';
...
interface DownloadState {
  ...
  addToQueue: (
    download: Omit<
      Download,
      "status" | "progress" | "downloadedBytes" | "addedAt"
    >,
  ) => Promise<void>;
  updateProgress: (
    id: string,
...
export const useDownloadStore = create<DownloadState>((set, get) => ({
  ...
  addToQueue: async (download) => {
    ...
    try {
      ...
      set((state) => ({
        downloads: [
          newDownload,
          ...state.downloads.filter((d) => d.id !== newDownload.id),
        ],
      }));
    } catch (error) {
  ...
  updateProgress: (id, progress, downloadedBytes) => {
    ...
    if (
      download &&
      Math.floor(progress / 10) !== Math.floor((download.progress || 0) / 10)
    ) {
      db.runSync(
        `
