...
class WebSocketService {
  ...
  private readonly MAX_RECONNECT_DELAY = 60000; // 1 minute cap

  // Injected dependencies (set from App.tsx startup)
  private deps: {
    jellyfinApi?: any;
    playerStore?: any;
  ...
  setDependencies(deps: typeof this.deps) {
    this.deps = { ...this.deps, ...deps };
  }

  async connect() {
    this.reconnectAttempts = 0; // Reset backoff on explicit connect
    ...
    const waitForDeviceIdFn = this.deps.waitForDeviceId || require("../api/jellyfin").waitForDeviceId;
    const deviceId = await waitForDeviceIdFn();
    const wsUrl =
      ...
      deviceId +
      "&Client=" +
      encodeURIComponent("Jellyspot") +
      "&Device=" +
      encodeURIComponent("React Native") +
      "&Version=1.0.0";

    // Ensure capabilities are reported BEFORE connecting to the socket
    const jellyfinApiInstance = this.deps.jellyfinApi || require("../api/jellyfin").jellyfinApi;
    try {
      await jellyfinApiInstance.reportCapabilities();
    } catch (e) {
  ...
  private handleMessage(message: any) {
    ...
    switch (MessageType) {
      ...
      case "Sessions":
        ...
        const otherSessions = (Data as RemoteSession[]).filter(
