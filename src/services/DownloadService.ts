...
class DownloadService {
  ...
  async queueTrack(item: {
    ...
    if (
      store.downloads.some(
        (d) =>
          d.id === item.id && d.status !== "failed" && d.status !== "cancelled",
      )
    ) {
      return;
    }
  ...
  async startProcessing(): Promise<void> {
    ...
    try {
      while (this.isRunning) {
        const store = useDownloadStore.getState();
        const pendingDownloads = store.downloads.filter(
          (d) => d.status === "pending",
        );
  ...
  private async downloadTrack(download: Download): Promise<void> {
    ...
    try {
      ...
      const safeFilename =
        this.cleanFilename(`${download.artist} - ${download.name}`) + ".m4a";
      ...
      if (
        ...
        this.customDirectory.uri.startsWith("content://")
      ) {
        try {
          ...
          console.warn(
            "[DownloadService] SAF write failed, using internal storage:",
  ...
  private async addToLocalLibrary(
    download: Download,
    localPath: string,
  ): Promise<void> {
    try {
  ...
  private async showBatchProgressNotification(
    trackName: string,
    ...
    try {
      ...
      const completedDownloads = store.downloads.filter(
      ...
      const currentDownload = store.downloads.find(
